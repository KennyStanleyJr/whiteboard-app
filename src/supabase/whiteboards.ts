...
/** Max name length (enforced in app and DB). */
const NAME_MAX_LENGTH = 200

/** Bcrypt truncates at 72 bytes; cap password length to avoid silent truncation. */
...
type WhiteboardInsert = Omit<WhiteboardRow, 'id' | 'created_at'> & {
	updated_at: string
}

type WhiteboardSelect = Pick<
	WhiteboardRow,
	'id' | 'name' | 'password_hash' | 'created_at' | 'updated_at'
>

export type SaveWhiteboardParams = {
	name: string
	password?: string
	data: unknown
}
...
export type LoadWhiteboardResult =
	| { ok: true; data: unknown }
	| { ok: false; error: string }

export type ListWhiteboardItem = {
	id: string
	name: string
	hasPassword: boolean
	created_at: string
...
export type ListWhiteboardsResult =
	| { ok: true; items: ListWhiteboardItem[] }
	| { ok: false; error: string }
...
export async function saveWhiteboard(
	...
	if (name.length > NAME_MAX_LENGTH) {
		return { ok: false, error: `Name must be ${NAME_MAX_LENGTH} characters or less.` }
	}
	...
	const passwordHash =
		rawPassword.length > 0 ? hashPassword(rawPassword) : null

	const row: WhiteboardInsert = {
		name,
...
export async function loadWhiteboard(
	id: string,
	password?: string,
): Promise<LoadWhiteboardResult> {
	const supabase = getSupabaseClient()
	if (!supabase) {
		return { ok: false, error: 'Supabase is not configured.' }
	}
	if (!isValidUuid(id)) {
		return { ok: false, error: 'Whiteboard not found.' }
	}
	...
	if (row.password_hash) {
		...
		if (!verifyPassword(provided, row.password_hash)) {
...
export async function listWhiteboards(): Promise<ListWhiteboardsResult> {
	...
	const items: ListWhiteboardItem[] = rows.map((r) => ({
		id: r.id,
		name: r.name,
...
export async function deleteWhiteboard(
	id: string,
	password?: string,
): Promise<DeleteWhiteboardResult> {
	const supabase = getSupabaseClient()
	if (!supabase) {
		return { ok: false, error: 'Supabase is not configured.' }
	}
